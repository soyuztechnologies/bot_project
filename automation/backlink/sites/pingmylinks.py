"""
pingmylinks.py — https://www.pingmylinks.com/addurl/
Single URL input + submit. Result table row: url + status <img> ('ok').
"""
...
def submit(driver, site, target, config, stop_event=None, session_logger=None):
    ...
    success = bool(found) and ("ok" in lowered or "ping" in lowered or url.lower() in lowered or "img." in lowered)
    return {"success": success, "result_text": text, "result_xpath": used, "detail": f"status-img ok check on {site['id']}"}
